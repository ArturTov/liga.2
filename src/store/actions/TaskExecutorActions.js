	...
	async (_, thunkAPI) => {
		try {
			const response = await instance.get(
				"v1/user/show-all-tasks-to-executor",
			)
...
export const taskCountExecutor = createAsyncThunk(
	...
	async (_, thunkAPI) => {
		try {
			const response = await instance.get(
				"v1/user/executor-task-section-count/executor",
			)
			return response.data
		} catch (e) {
			console.log(e.response, "register error")
...
export const getResponseOrders = createAsyncThunk(
	...
	async (_, thunkAPI) => {
		try {
			const response = await instance.get(
				"v1/user/responded-task-for-executor",
			)
...
export const getWorkOrders = createAsyncThunk(
	"taskExecutor/workOrders",
	async (_, thunkAPI) => {
		try {
			const response = await instance.get(
				"v1/user/tasks-in-progress-for-executor",
			)
...
export const getNotConfirmedEmployer = createAsyncThunk(
	...
	async (_, thunkAPI) => {
		try {
			const response = await instance.get(
				"v1/user/special-task-for/executor",
			)
